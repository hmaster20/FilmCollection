...
namespace FilmCollection
{
    public partial class ucChart : UserControl
    {
        ...
        private GViewer gViewer { get; set; }
        ...
        private void redoLayout_Click(object sender, EventArgs e)
        {
            RedoLayoutRedoLayout();
        }

        internal virtual void RedoLayoutRedoLayout()
        {
        ...
        private void tsDetails_Click(object sender, EventArgs e)
        {
            if (gViewer.SelectedObject is Node)
            {
                ...
                if (_videoCollection.ActorList.Exists(x => x.FIO == node.Id) && _videoCollection.ActorList.First(x => x.FIO == node.Id).VideoID.Count > 0)
                {
                    ...
                    {
                        if (_videoCollection.CombineList.Exists(x => x.media.Id == ListID))
                        {
